...
- Important: Submit ImGui widgets only when window content is visible : `if(ImGui::Begin("MyWindow")){ /* Draw widgets here... */ } ImGui::End();`  
  (_Any widget sumbitted outside of this scope will show up in the "Debug" window_)
...
When you run your binary in debug mode, ImGui will often warn you about misimplemented widgets (example: when you forget a `PopXXX()` after a `PushXXX()`). _These warnings can be asserts (informative app crashes) or GUI dialogs (red)._
