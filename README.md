...
If your project is requiring a special setup (or you wish to fine-tune the GUI) you might need to manually set some configuration options, which are explained in [Configure.md](./Configure.md).
...
- Call `mygui.setup()`, you can pass a few arguments to setup ofxImGui to your needs :  
...
*For more experienced ImGui users: You can even write "ImGui extensions" which have access to an additional ImGuiInternal API.*
...
(*Any widget sumbitted outside of this scope will show up in the "Debug" window*)
...
In your ofApp::draw(), call `ofAppGui.begin();` and `ofAppGui.end();`. In-between you can submit your GUI components.

The ImGui API is very flexible : you can use try and error to learn.  
When you run your binary in debug mode, ImGui will often warn you about misimplemented widgets (example: when you forget a `PopXXX()` after a `PushXXX()`). *These warnings can be asserts (informative app crashes) or GUI dialogs (red).*

**Important**: Each **interactive** imgui widget needs to have a unique ID within the ImGui ID stack. This can be a `char`* or a `void`* that has a static memory address (that won't move over time). Most widgets take a unique label that also serves as an ID, but they can be fed with an explicit ID if your labels are identical. Non-interactive widgets don't need an ID.  
For example, using `std::list` is fine: you can use the item as a pointer, but not for `std::vector` as items can be reallocated/moved on the fly.  
Nowadays, ImGui provides some ID stack debug tools and warnings that minimise the ID mistakes that can lead to unexpected behaviour.
